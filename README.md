...
- Current branch notes and exact pinned versions are tracked in [Changelog](Changelog) and [VERSIONS](VERSIONS).
...
- **Interactive WPF GUI (default).** With no action switch and without `-NoGui`, a WPF launcher window appears with radio buttons for the action (Preflight only, Run training now, Register scheduled task, Register task and run now, Unregister scheduled task, Update toolchain), an [AI Provider](#ai-powered-features) group (Ollama, OpenAI, Anthropic, Azure OpenAI) and a dependent model dropdown, an editable Ollama-host combo (enabled only for Ollama), a scheduler policy group with explicit interactive / idle-only / unattended choices, Azure endpoint/deployment fields (shown only for Azure), schedule frequency/time dropdowns (Weekly switches the time dropdown to weekday names), a masked `PasswordBox` for API keys, an "Enable Task Scheduler history" checkbox, and **OK / Cancel / Save as Defaults** buttons. Save-as-Defaults persists the documented non-secret fields to `%LOCALAPPDATA%\myTech.Today\autoresearch-win-rtx-scheduled\launch.json`. The file is human-readable JSON in the current user's local application-data directory, not an encrypted secret store; both the GUI and `-NoGui` CLI read it so the two surfaces behave identically (explicit CLI parameters always override stored values). A legacy `scripts\launch.json` beside the canonical launcher is read for compatibility but is never written by the current launcher.
...
### Launcher defaults and access model

The defaults file is intentionally human-readable and stores only the operational selections needed to make the GUI and `-NoGui` CLI round-trip consistently: `Provider`, `Model`, `OllamaHost`, `AzureEndpoint`, `AzureDeployment`, `LogDir`, `ScheduleFrequency`, `ScheduleTime`, and `SchedulerPolicy`. It is written under the current user's `%LOCALAPPDATA%` rather than beside the shared canonical launcher, so the file follows the user's local application-data permissions. It is still not encrypted and may be readable by local administrators or system backup tooling; treat it as operational metadata, not a secret store. `ApiKey`, GUI action state, and task-history checkbox state are never persisted. Supply the API key via the masked GUI field or the `-ApiKey` parameter at invocation time. A legacy `scripts\launch.json` is read but never written, allowing existing settings to continue working after the per-user location change.
...
- **Optimizer invariant enforcement.** `train.py` raises `OptimizerSetupError` if optimizer parameter grouping ever drifts from the model's parameter set, so the consistency check still fires under `python -O`.
